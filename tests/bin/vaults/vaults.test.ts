...
import type { VaultId, Vault, VaultName } from '@/vaults/types';
import os from 'os';
...
describe('CLI vaults', () => {
  ...
  test(
    ...
    async () => {
      ...
      await targetPolykeyAgent.nodeManager.setNode(
        polykeyAgent.nodeManager.getNodeId(),
        {
      ...
      command = ['vaults', 'clone', '-np', dataDir, vaultName, targetNodeId];
      ...
      await polykeyAgent.vaultManager.withVaults(
        ...
        async (secondClonedVault) => {
          const file = await secondClonedVault.access(async (efs) => {
            return await efs.readFile('secret 1', { encoding: 'utf8' });
          });
          expect(file).toBe('secret the first');
        },
  ...
  test(
    ...
    async () => {
      ...
      command = ['vaults', 'share', '-np', dataDir, vaultName, targetNodeId];
      let result = await testBinUtils.pkStdio([...command], {}, dataDir);
      expect(result.exitCode).toBe(0);
  ...
  describe('commandVaultVersion', () => {
    ...
    test('should handle invalid version IDs', async () => {
      ...
      expect(result.stderr).toContain('ErrorVaultsCommitUndefined');
    });
