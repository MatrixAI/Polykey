...
describe('ForwardProxy', () => {
  ...
  // @ts-ignore
  console.log(global.globalDataDir);

  const logger = new Logger('ForwardProxy Test', LogLevel.WARN, [
    new StreamHandler(),
  ]);
  const keyPairPem = testUtils.globalKeyPairPem;
  const cert = keysUtils.generateCertificate(
    testUtils.globalKeyPair.publicKey,
    testUtils.globalKeyPair.privateKey,
    testUtils.globalKeyPair.privateKey,
    86400
  );
  ...
  //   await fwdProxy.stop();
  // });
  // test.only('open connection fails due to missing certificates', async () => {
