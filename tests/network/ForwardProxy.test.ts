...
describe('ForwardProxy', () => {
  ...
  test(
    'open connection reset due to ending remote',
    async () => {
      const authToken = 'sdafjs8';
      const fwdProxy = new ForwardProxy({
        authToken,
  ...
  test(
    'open connection fails due to invalid node id',
    async () => {
      ...
      const egressHost = fwdProxy.getEgressHost();
      const egressPort = fwdProxy.getEgressPort();
      const { p: remoteReadyP, resolveP: resolveRemoteReadyP } =
        promise<void>();
      const { p: remoteClosedP, resolveP: resolveRemoteClosedP } =
        promise<void>();
      // this UTP server will hold the connection
  ...
  test(
    'open connection to multiple servers',
    async () => {
      // first server keys
