...
describe('tokens/schemas', () => {
  testProp(
    ...
    (
      ...
      expect(tokensSchemas.validateSignedTokenEncoded(signedTokenEncodedCorrect)).toBe(true);
      expect(tokensSchemas.validateSignedTokenEncoded.errors).toBeNull();
      expect(tokensSchemas.validateSignedTokenEncoded(signedTokenEncodedIncorrect)).toBe(false);
      expect(tokensSchemas.validateSignedTokenEncoded.errors).not.toBeNull();
      expect(tokensSchemas.validateSignedTokenEncoded.errors!.length).toBeGreaterThan(0);
    }
  );
});
