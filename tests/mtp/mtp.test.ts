...
import { MTPServer, MTPConnection } from '../../src/network/micro-transport-protocol/MTPServer'
...
describe('micro transport protocol', () => {
  test('simple - server client', done => {
    new MTPServer(conn => {
      ...
      const connection = MTPConnection.connect('bacebd0a4af7939e1c5ae8a301bd757d', a.port, a.host);
  ...
  test('existing server socket - server client', done => {
    ...
    existingServerSocket.bind(0, 'localhost', () => {
      ...
      }, new Logger('Mock Logger')).listenSocket(existingServerSocket, (a) => {
        const connection = MTPConnection.connect('bacebd0a4af7939e1c5ae8a301bd757d', a.port, a.host);
        connection.write('client');
        connection.on('data', (data) => {
          expect(data.toString()).toEqual('server');
  ...
  test('existing client socket', done => {
    ...
    existingClientSocket.bind(0, 'localhost', () => {
      new MTPServer(conn => {
        ...
        const connection = MTPConnection.connect('bacebd0a4af7939e1c5ae8a301bd757d', a.port, a.host, existingClientSocket);
        connection.write('client');
        connection.on('data', (data) => {
          expect(data.toString()).toEqual('server');
          done()
  ...
  test('bulk sequence writing - 256 byte packet length - server client', done => {
    const buffer: Buffer[] = [...Array(50).keys()].map(k => crypto.randomBytes(256));
    new MTPServer(conn => {
      let serverStep = 0;
      conn.on('data', (data) => {
      ...
      for (const [i, buf] of buffer.entries()) {
        connection.write(buf);
      }
    })
  })
  test('bulk sequence writing - 1024 byte packet length - server client', done => {
    const buffer: Buffer[] = [...Array(50).keys()].map(k => crypto.randomBytes(1024));
    new MTPServer(conn => {
      let serverStep = 0;
      conn.on('data', (data) => {
        expect(data).toEqual(buffer[serverStep]);
  ...
  test('big packets - 16384 byte packet length - server client', done => {
    ...
    new MTPServer(conn => {
      ...
      conn.on('data', (data: Buffer) => {
        packetsReceived += 1
        expect(data).toEqual(buffer.slice(serverBytesRead, serverBytesRead + data.length));
        conn.write(data);
      ...
      connection.on('data', (data: Buffer) => {
        expect(data).toEqual(buffer.slice(clientBytesRead, clientBytesRead + data.length));
