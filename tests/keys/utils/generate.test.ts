...
describe('keys/utils/generate', () => {
  ...
  test('generate key pair', () => {
    ...
    expect(keyPair1.secretKey).toStrictEqual(Buffer.concat([keyPair1.privateKey, keyPair1.publicKey]));
    expect(keyPair2.publicKey).not.toEqual(keyPair2.privateKey);
    expect(keyPair2.secretKey).toStrictEqual(Buffer.concat([keyPair2.privateKey, keyPair2.publicKey]));
