...
describe('context/decorators/timed', () => {
  test('timed decorator', async () => {
    const s = Symbol('sym');
    class X {
      a(ctx?: { signal?: AbortSignal; timer?: Timer }, check?: (t: Timer) => any): void;
      @timed(1000)
      a(@context ctx: { signal: AbortSignal; timer: Timer }, check?: (t: Timer) => any): void {
        expect(ctx.signal).toBeInstanceOf(AbortSignal);
        expect(ctx.timer).toBeInstanceOf(Timer);
        if (check != null) check(ctx.timer);
      }

      b(ctx?: { signal?: AbortSignal; timer?: Timer }, check?: (t: Timer) => any): Promise<void>;
      @timed(Infinity)
      async b(@context ctx: { signal: AbortSignal; timer: Timer }, check?: (t: Timer) => any): Promise<void> {
        expect(ctx.signal).toBeInstanceOf(AbortSignal);
        expect(ctx.timer).toBeInstanceOf(Timer);
        if (check != null) check(ctx.timer);
      }

      c(ctx?: { signal?: AbortSignal; timer?: Timer }, check?: (t: Timer) => any): Generator<void>;
      @timed(0)
      *c(@context ctx: { signal: AbortSignal; timer: Timer }, check?: (t: Timer) => any): Generator<void> {
        expect(ctx.signal).toBeInstanceOf(AbortSignal);
        expect(ctx.timer).toBeInstanceOf(Timer);
        if (check != null) check(ctx.timer);
      }

      d(ctx?: { signal?: AbortSignal; timer?: Timer }, check?: (t: Timer) => any): AsyncGenerator<void>;
      @timed(NaN)
      async *d(@context ctx: { signal: AbortSignal; timer: Timer }, check?: (t: Timer) => any): AsyncGenerator<void> {
        expect(ctx.signal).toBeInstanceOf(AbortSignal);
        expect(ctx.timer).toBeInstanceOf(Timer);
        if (check != null) check(ctx.timer);
      }

      [s](ctx?: { signal?: AbortSignal; timer?: Timer }, check?: (t: Timer) => any): void;
      @timed()
      [s](@context ctx: { signal: AbortSignal; timer: Timer }, check?: (t: Timer) => any): void {
        expect(ctx.signal).toBeInstanceOf(AbortSignal);
        expect(ctx.timer).toBeInstanceOf(Timer);
        if (check != null) check(ctx.timer);
      }
