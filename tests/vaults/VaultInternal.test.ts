...
describe('VaultInternal', () => {
  ...
  test('VaultInternal readiness', async () => {
    ...
    await expect(async () => {
      await vault.log();
    }).rejects.toThrow(vaultsErrors.ErrorVaultsVaultDestroyed);
  });
  ...
  test('does not allow changing to an unrecognised commit', async () => {
    await expect(() => vault.version('unrecognisedcommit')).rejects.toThrow(
      vaultsErrors.ErrorVaultsCommitUndefined,
    );
    await vault.commit(async (efs) => {
      await efs.writeFile('test1', 'testdata1');
    });
  ...
  test('cannot commit when the remote field is set', async () => {
    ...
    const commit = (await vault.log(1))[0];
    await vault.version(commit.oid);
    const files = await vault.access(async (efs) => {
      return await efs.readdir('.');
    });
    ...
    ).rejects.toThrow(vaultsErrors.ErrorVaultsVaultImmutable);
  });
});
