...
import { GestaltIdentityInfo, GestaltNodeInfo } from '../../src/gestalts/types';
import Token from '@/tokens/Token';
import { parseSignedClaimLinkNode, assertClaimLinkNode } from '../../src/claims/payloads/index';
import { ClaimIdEncoded } from '@/claims/types';
...
describe('GestaltGraph', () => {
  ...
  const nodeInfoABC: GestaltNodeInfo = {
    nodeId: nodeIdABC,
  };
  const nodeIdABCEncoded = nodesUtils.encodeNodeId(nodeIdABC);
  const encodedGestaltNodeIdABC = gestaltsUtils.encodeGestaltNodeId(['node', nodeIdABC]);
  const nodeIdDEE = testNodesUtils.generateRandomNodeId();
  const nodeInfoDEE: GestaltNodeInfo = {
    nodeId: nodeIdDEE,
  };
  const nodeIdDEEEncoded = nodesUtils.encodeNodeId(nodeIdDEE);
  const encodedGestaltNodeIdDEE = gestaltsUtils.encodeGestaltNodeId(['node', nodeIdDEE]);
  ...
  const identityInfo: GestaltIdentityInfo = {
  ...
  ]; const encodeGestaltIdentityId = gestaltsUtils.encodeGestaltIdentityId([
    'identity',
    providerIdentityId,
  ]);
  ...
  test('gestaltGraph readiness', async () => {
    ...
    const getGestalts = async () => {
      for await (const item of gestaltGraph.getGestalts()){
  ...
  test('get, set and unset node', async () => {
    ...
    try {
      await gestaltGraph.setNode(nodeInfoABC);
      const gestalt = await gestaltGraph.getGestaltByNode(nodeIdABC);
      expect(gestalt).toStrictEqual({
        matrix: { [encodedGestaltNodeIdABC]: {} },
        nodes: {
          [encodedGestaltNodeIdABC]: {
            nodeId: nodeIdABC,
          },
        },
        identities: {},
  ...
  test('get, set and unset identity', async () => {
    ...
    try {
      ...
      const gestalt = await gestaltGraph.getGestaltByIdentity(providerIdentityId);
      expect(gestalt).toStrictEqual({
        matrix: { [encodeGestaltIdentityId]: {} },
        nodes: {},
        identities: { [encodeGestaltIdentityId]: identityInfo },
      });
  ...
  test('setting independent node and identity gestalts', async () => {
    ...
    try {
      ...
      const gestaltIdentity = await gestaltGraph.getGestaltByIdentity(providerIdentityId);
  ...
  test('link node to node', async () => {
    ...
    try{
      ...
      // dee -> abc
      await gestaltGraph.setNode(nodeInfoABC);
      await gestaltGraph.setNode(nodeInfoDEE);
      const claim = Token.fromPayload({
        iss: nodeIdABCEncoded,
        sub: nodeIdDEEEncoded,
      ...
      claim.signWithKey(key)
      await gestaltGraph.linkNodeAndNode(nodeInfoABC, nodeInfoDEE, {
        claim: claim.toSigned(),
        meta: {}
      });
      ...
      expect(gestaltNode1).toStrictEqual({
        matrix: {
          [encodedGestaltNodeIdABC]: {
            [encodedGestaltNodeIdDEE]: expect.any(Array),
          },
          [encodedGestaltNodeIdDEE]: {
            [encodedGestaltNodeIdABC]: expect.any(Array),
          },
        },
        nodes: {
          [encodedGestaltNodeIdABC]: {
  ...
  //     // abc -> GitHub
  //     // GitHub -> abc
  //     await gestaltGraph.linkNodeAndIdentity(nodeInfoABC, identityInfo, {
  //       claim: undefined,
  //       meta: {
