...
class PeerInfo {
  ...
  constructor(
    ...
    // this.createNewPeerInfoCertificate = createNewPeerInfoCertificate
  ...
  static fromX509Pem(str: string): PeerInfo {
    const cert = pki.certificateFromPem(str)

    const publicKey = cert.subject.attributes['1.3.1.4.1'].value
    const peerAddress = cert.subject.attributes['1.3.1.4.2'].value
    const apiAddress = cert.subject.attributes['1.3.1.4.3'].value
    return new PeerInfo(publicKey, str, peerAddress, apiAddress)
  }
}
