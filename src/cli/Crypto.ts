...
function makeSignCommand() {
  ...
  .action(async (options) => {
    ...
    for (const filePath of filePathList) {
      try {
        const signedPath = await pk.keyManager.signFile(filePath, signingKeyPath, keyPassphrase)
...
function makeVerifyCommand() {
  ...
  .action(actionRunner(async (options) => {
    ...
    try {
      const pgpFingerprint = await pk.keyManager.verifyFile(filePath, detachedSignaturePath, verifyingKeyPath)
