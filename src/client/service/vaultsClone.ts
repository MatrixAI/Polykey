...
function vaultsClone({
  ...
  return async (
    ...
    try {
      ...
      // Vault id
      let vaultId;
      vaultId = vaultMessage.getNameOrId();
      try {
        vaultId = vaultsUtils.makeVaultId(vaultId);
      } catch (err) {
        vaultId = vaultId as VaultName;
      }
      // Node id
      const nodeId = nodesUtils.makeNodeId(nodeMessage.getNodeId());
