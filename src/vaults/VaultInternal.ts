...
class VaultInternal {
  ...
  public async start({
    ...
    this.logger.info(`Starting ${this.constructor.name} - ${this.vaultIdEncoded}`);
    const vaultDbDomain = [...this.vaultsDbDomain, this.vaultIdEncoded];;
    const vaultDb = await this.db.level(
      this.vaultIdEncoded,
      this.vaultsDb
    );
    if (fresh) {
    ...
    this.logger.info(`Started ${this.constructor.name} - ${this.vaultIdEncoded}`);
  }

  public async stop(): Promise<void> {
    this.logger.info(`Stopping ${this.constructor.name} - ${this.vaultIdEncoded}`);
    this.logger.info(`Stopped ${this.constructor.name} - ${this.vaultIdEncoded}`);
  }
