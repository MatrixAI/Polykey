...
class VaultManager {
  ...
  public async destroyVault(vaultName: VaultName) {
    ...
    const vaultLock = this.vaultsMap.get(vaultName);
    if (vaultLock == null) {
      return
    }
    await this._transaction(async () => {
      ...
      this.vaultsMap.delete(vaultName);
    }, vaultLock.lock);
  }
  ...
  protected async getVault(vaultName: VaultName): Promise<VaultInternal> {
    ...
    let vaultAndLock = this.vaultsMap.get(vaultName);
    if (vaultAndLock != null) {
      ...
      try {
        ...
        this.vaultsMap.set(vaultName, vaultAndLock);
        return vault;
      } finally {
        release();
      }
