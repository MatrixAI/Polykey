...
class Vault {
  ...
  public async create() {
    ...
    this.efs.mkdirSync(path.join(this.baseDir, this.vaultId), {
      recursive: true,
    });
    this.logger.info(
  ...
  public async destroy() {
    await this.efs.promises.rmdir(path.join(this.baseDir, this.vaultId), {
      recursive: true,
    });
    this.logger.info(
  ...
  public async initializeVault(): Promise<void> {
    ...
    await git.init({
      fs: fileSystem,
      dir: path.join(this.baseDir, this.vaultId),
    });
    ...
    efs.writeFileSync(
      path.join(path.join(this.baseDir, this.vaultId), '.git', 'packed-refs'),
  ...
  public async vaultStats(): Promise<fs.Stats> {
    ...
    try {
      return this.efs.statSync(path.join(this.baseDir, this.vaultId));
    } finally {
      release();
    }
  }
  ...
  public async mkdir(
    ...
    const newDir = path.join(this.baseDir, this.vaultId, dirPath);
    const release = await this.mutex.acquire();
    try {
  ...
  public async addSecret(
    ...
    const secretPath = path.join(this.baseDir, this.vaultId, secretName);
    const release = await this.mutex.acquire();
    try {
      if (!this.efs.existsSync(path.join(this.baseDir, this.vaultId, '.git'))) {
  ...
  public async addSecretDirectory(secretDirectory: string): Promise<void> {
    ...
    try {
      ...
      for await (const secretPath of vaultsUtils.readdirRecursively(
        ...
        if (
          !this.efs.existsSync(path.join(this.baseDir, this.vaultId, '.git'))
        ) {
          throw new errors.ErrorVaultUninitialised(
            `${this.vaultName} has not been initialised\nVaultId: ${this.vaultId}`,
          );
        } else if (
          this.efs.existsSync(path.join(this.baseDir, this.vaultId, secretName))
        ) {
          const writePath = path.join(this.baseDir, this.vaultId, secretName);
          // Write secret
  ...
  public async updateSecret(
    ...
    try {
      ...
      if (!fs.existsSync(path.join(this.baseDir, this.vaultId, secretName))) {
  ...
  public async renameSecret(
    ...
    try {
      ...
      await this.efs.promises.rename(
        path.join(this.baseDir, this.vaultId, currSecretName),
        path.join(this.baseDir, this.vaultId, newSecretName),
      );
    ...
    if (
      fs.existsSync(path.join(this.baseDir, this.vaultId, currSecretName)) ||
      !fs.existsSync(path.join(this.baseDir, this.vaultId, newSecretName))
    ) {
      return false;
    }
    return true;
  ...
  public async listSecrets(): Promise<Array<string>> {
    ...
    for await (const secret of vaultsUtils.readdirRecursivelyEFS(
      this.efs,
      path.join(this.baseDir, this.vaultId),
    )) {
      if (path.basename(secret) != 'nodePermissions') {
        secrets.push(
          path.relative(path.join(this.baseDir, this.vaultId), secret),
        );
      }
    }
    return secrets;
