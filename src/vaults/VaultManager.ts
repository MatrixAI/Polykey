...
import { errors, mkdirExists } from '../utils';
...
class VaultManager {
  ...
  public async start({
    ...
    try {
      ...
      const vaultsDb = await this.db.level(
        this.vaultsDbDomain[0]
      );
  ...
  public async listVaults(): Promise<VaultList> {
    ...
    for await (const o of this.vaultsMetaDb.createReadStream({})) {
      ...
      vaults.set(vaultMeta.name, vaultsUtils.makeVaultId(dbId));
    }
    return vaults;
  }
  ...
  public async getVaultId(vaultName: VaultName): Promise<VaultId | undefined> {
    ...
    for await (const o of this.vaultsMetaDb.createReadStream({})) {
      ...
      if (vaultName === vaultMeta.name) {
        return vaultsUtils.makeVaultId(dbId);
      }
    }
  }
  ...
  public async pullVault({
    ...
    return await this.transact(async () => {
      ...
      if (pullVaultNameOrId == null) {
        pullVaultNameOrId = idUtils.fromString(vaultMeta.remoteVault!);
      } else {
        metaChange = 1;
      ...
      const client = nodeConnection.getClient();
      const [request, _, remoteVaultId] = await vaultsUtils.request(
        client,
        thisNodeId,
        pullVaultNameOrId!,
      );
