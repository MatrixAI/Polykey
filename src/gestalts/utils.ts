...
function protobufToGestalt(message: agent.GestaltMessage): Gestalt {
  ...
  message.getGestaltMatrixMap().toArray().forEach(([key, encodedEdge]) => {
    const edge: {[key: string]: LinkInfo} = {}
    encodedEdge.getPairsMap().toArray().forEach(([innerKey, linkInfoMessage]) => {
  ...
  message.getIdentitiesMap().toArray().forEach(([key, identityInfoMessage]) => {
  ...
  message.getGestaltNodesMap().toArray().forEach(([key, gestaltNodeMessage]) => {
    gestalt.nodes[key] = {
