...
export class GitPackIndex {
  ...
  static async fromPack ({ pack, getExternalRefDelta }) {
    ...
    await new Promise((resolve, reject) => {
      buffer2stream(pack)
        .pipe(listpack())
        .on('data', async (data, type, reference, offset, num: number) => {
    ...
    for (let offset in offsetToObject) {
      ...
      let o = offsetToObject[numberOffset]
      if (o.oid) continue
      try {
        p.readDepth = 0
        p.externalReadDepth = 0
        marky.mark('readSlice')
        let { type, object } = await p.readSlice({ start: numberOffset })
  ...
  async readSlice ({ start }) {
    ...
    if (object.byteLength !== length) {
      throw(new Error(`Packfile told us object would have length ${length} but it had length ${
        object.byteLength
      }`))
    }
    if (base) {
