...
commandSearch.action(
  actionRunner(async (options) => {
    ...
    pkLogger.logV1(`searching for connected identities with search terms: ${commandSearch.args}`, PKMessageType.INFO);
    await new Promise<void>((resolve, reject) => {
      responseStream.on('data', (identityInfo: agentPB.IdentityInfoMessage) => {
        ...
        pkLogger.logV1(`provider: '${identityInfo.getProvider()}'`, PKMessageType.INFO);
        pkLogger.logV1(`name: '${identityInfo.getName()}'`, PKMessageType.INFO);
        pkLogger.logV1(`email: '${identityInfo.getEmail()}'`, PKMessageType.INFO);
    ...
    pkLogger.logV1('finished searching for connected identities', PKMessageType.INFO);
  }),
);
...
commandDiscoverIdentity.action(
  actionRunner(async (options) => {
    ...
    await new Promise<void>((resolve, reject) => {
      responseStream.on('data', (_: agentPB.EmptyMessage) => {
...
commandGetByIdentity.description(
  'retrieve a gestalt via an identity',
);
...
commandGetByIdentity.action(
  actionRunner(async (options) => {
    ...
    request.setProviderKey(options.providerKey);
    request.setKey(options.identityKey);
    const response = (await promisifyGrpc(client.getGestaltByIdentity.bind(client))(
      request,
...
commandTrust.description(
  'trust a particular gestalt',
);
commandTrust.option(
  '-k, --node-path <nodePath>',
  'provide the polykey path',
...
commandUntrust.description(
  'untrust a particular gestalt',
);
...
commandTrusted.description(
  'check if a particular gestalt is trusted',
);
...
commandTrusted.action(
  actionRunner(async (options) => {
    ...
    pkLogger.logV1(`gestalt is ${response.getB() ? '' : 'un '}trusted`, PKMessageType.SUCCESS);
  }),
);

const commandGetGestalts = new commander.Command('list');
commandGetGestalts.alias('ls');
commandGetGestalts.description(
  'list all the gestalts in the gestalt graph',
);
