...
const reset = binUtils.createCommand('reset', {
  description: 'Forces a reset of a new root keypair which will be self-signed',
  nodePath: true,
  verbose: true,
  format: true,
});
reset.requiredOption(
  '-pp, --password-path <passwordPath>',
  '(required) File path to the passphrase to the new root keypair',
);
