...
class Network {
  ...
  constructor(
    ...
    this.mtpServer = new MTPServer(this.connectionHandler.bind(this), this.logger);
  }
    ...
    return new Promise<Address>(async (resolve, reject) => {
      try {
        ...
        this.mtpServer.listenPort(port, host, async () => {
          ...
          this.logger.info(`main MTP server is now listening on address: '${address.toString()}'`);
  ...
  private connectionHandler(conn: MTPConnection) {
    ...
    const pipe = new MTPToTCPSocketPipe(conn, this.getLocalNodeInfo().nodeAddress!)
